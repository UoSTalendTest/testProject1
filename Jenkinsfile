...
pipeline {
	...
	withEnv(["JAVA_HOME=${ tool 'Java 8' }", 
		 "PATH+MAVEN=${tool 'Maven'}/bin:${env.JAVA_HOME}/bin"]) {
			sh "mvn --batch-mode -V -U -e  -Dsurefire.useFile=false -f /opt/talend-jenkins/ci-builder-pom.xml --settings "${talend_home}"/studio/configuration/maven_user_settings.xml org.talend:ci.builder:6.4.1:local-generate"
		    }
	}
		}
		stage('Test Coverage') {
